$(document).ready(function(){
    // 메인 메뉴에 마우스 오버했을때 서브 배경 슬라이드 다운
    // hover이벤트는 유일하게 마우스 오버, 마우스 아웃 이벤트를 가지고 있음
    $('header .box nav > ul > li').hover(function(){
    ...
    // .btn a에 클릭이벤트 설정, .btn a를 클릭ㅎ면 자동 슬라이드가 멈추고, 다시 클릭하면 자동 슬라이드 실행, 버튼 모양도 바뀜
    var sw=0;
    // 스위치 변수 : 버튼 1개 클릭할때마다 상태가 변할 때 사용
    ...
    // fn 함수 선언
    function fn (){
        // .num ul li의 인덱스번호를 1씩 증가 시킴
        numLi += 1;
        // 만약 인덱스번호가 2보다 커지면 0으로 초기화
        if(numLi > 2) { numLi=0; }
        // numli 인덱스번호에 해당하는 li 클릭
    ...
    $('.product_list ul li').click(function(){
        ...
        $(this).addClass('active');
        // li의 자식인 a태그의 herf속성을 막음
        // e.preventDefault()메서드와 같은 역활
        return false;
    });
    // 자동슬라이드 (마우스 오버 했을때)
